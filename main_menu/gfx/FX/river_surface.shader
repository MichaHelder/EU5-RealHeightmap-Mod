...
PixelShader =
{
	...
	MainCode PS_surface
	{
		...
		[[	
			...
			{
				float Winterness = GetSnowAmountForWater( Water._Normal, Input.WorldSpacePos, ClimateMap );
				if( Winterness > 0.0f )
				{
					...
					float4 Diffuse = PdxTex2D( DetailDiffuseSnow, UV );
					...
					{
						float3 Normal = UnpackRRxGNormal( PdxTex2D( DetailNormalSnow, UV ) ).xzy;
						float4 Properties = PdxTex2D( DetailPropertiesSnow, UV );
			...
			{		
				...
				if( FlatMapBlend.x > 0.0f )
				{
					...
					RiverColor.a *= 1.5f; // - GetSimulatedFlatMapLerp();
