...
PixelShader =
{
	...
	float CalculateStripeMaskCustom( in float2 UV, float Offset )
	{
		...
		void ApplyDiagonalStripesCustom( inout float4 BaseColor, float4 StripeColor, float ShadowAmount, float2 WorldSpacePosXZ )
		{
			float Mask = CalculateStripeMaskCustom( WorldSpacePosXZ, 0.0f );
			float OffsetMask = CalculateStripeMaskCustom( WorldSpacePosXZ, -0.5f );
		...
		void ApplySecondaryProvinceOverlayCustom( in float2 NormalizedCoordinate, in float DistanceFieldValue, inout float4 Color )
		{
			...
			ApplyDiagonalStripesCustom( Color, SecondaryColor, 0.8, NormalizedCoordinate );
		}
		...
		{	
			...
			float3 BorderColorWithGreyscale = GetOverlay( BorderColor, Greyscale, 1.0 );
		...
		{	
			...
			float4 DetailColor = float4( FlatMapColorCoast, Base._Gradient );
			float2 CoastDetailUV =  float2( ColorMapCoords.x, 1.0 - ColorMapCoords.y ); //TODO: aspect ratio
			CoastDetailUV *= FlatMapCoastDetailTiles;
			CoastDetailUV = RotateUV( CoastDetailUV, vec2(0.5), 45 );//TODO: needed? can't we just rotate in texture instead. Saves instructions and sampling
			float CoastDetail = PdxTex2D( FlatMapDetailsTex, CoastDetailUV ).a;
			DetailColor.rgb = lerp( DetailColor.rgb, FlatMapColorDetail, CoastDetail * Base._Gradient );
			...
			TerrainSample.a = smoothstep( 0.0, FlatMapHeight_BlendRange, SampleHeight - BaseHeight ) * ( 1.0f );
			...
			// Fake Normalmap
			if ( 1.0f > 0.0f ) {
				float AccurateHeightL = GetHeight( float2(WorldSpacePos.x-1.0f, WorldSpacePos.z ) );
				float AccurateHeightR = GetHeight( float2(WorldSpacePos.x+1.0f, WorldSpacePos.z ) );
				float AccurateHeightD = GetHeight( float2(WorldSpacePos.x, WorldSpacePos.z-1.0f ) );
				float AccurateHeightU = GetHeight( float2(WorldSpacePos.x, WorldSpacePos.z+1.0f ) );
				
				float dX = AccurateHeightR - AccurateHeightL; // Gradient in X
				float dY = AccurateHeightU - AccurateHeightD; // Gradient in Y
				
				float3 normal = normalize(float3(dX, dY, 3.2)); // Z = 1 for smoothness 1.8 stronger
				
				float lighting = max(0.3, dot(normal, normalize(float3(-0.7, 0.7, 1.0)))); // Clamp to [0, 1]	
			...
			SeaColor = Multiply( SeaColor, DetailColor.rgb, DetailColor.a );
			float3 FinalColor = lerp( SeaColor, LandColor, Base._LandMask );

			FinalColor.rgb *= lerp( Detail.g, Detail.r, Base._LandMask );
